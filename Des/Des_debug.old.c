...
#if (DEBUG)
#define DEBUG_INIPERMUTE 0
#define DEBUG_ENSONKEY 0
#define DEBUG_ENFFUNC 0
...
void IniPermutate(unsigned char *plainText);     //初始置换
//void DepartLeftRight(unsigned char *cryptoText);                                                  //将64bit信息左右替换
...
//void Controller(unsigned char *cryptoText, unsigned char *sonKey8_48, int count);                 //迭代控制器void Cp8chars(unsigned char * newChar,unsigned char * origin char) {
//void Cp8chars(unsigned char *newChar, unsigned char *originChar);                                 //复制8字
...
int main(int argc, char *argv[])
{
    ...
    int i;
#if (DEBUG_INIPERMUTE)
    unsigned char *testText = (unsigned char *)malloc(8);
    strcpy(testText, "abcdefgh");
    IniPermutate(testText);
    DeIniPeimutate(testText);
    printf("text is :%s\n", testText);
#endif
#if (DEBUG_ENSONKEY)
    unsigned char *testKey = (unsigned char *)malloc(8);
    int j;
    strcpy(testKey, "abcdefgh");
    testKey = EnKeySon(testKey);
    for (i = 0; i < 16; i++)
    {
        printf("key %d is \t", i);
        for (j = 0; j < 6; j++)
        {
            printf("0x%x ", *(testKey + 6 * i + j));
        }
        printf("\n");
    }
#endif
    ...
    unsigned char *testText = (unsigned char *)malloc(8);
    strcpy(testText, "abcdefgh");
    EnFFunc(testText, testText + 4, testKey, 0);
    printf("\n");
    printf("cipherText is :\t");
    for (i = 0; i < 8; i++)
    {
        printf("0x%x ", *(testText + i));
    }
#endif
#if (DEBUG_GENERAL)
    int size;
...
unsigned char *EnCryptoDes(char *plainText, char *secretKey, int numOf64)
{
    ...
    //预处理
    //numOf64 = strlen(plainText) / 8 + (strlen(plainText) / 8) ? 1 : 0;
    ...
    for (count = 0; count < numOf64; count++)
    {
        ...
        //CpChars(cryptoText, plainText + count * 8, 8);
        //CpChars(cryptoText, plainText + count * 8,8);
...
unsigned char *DeCryptoDes(char *cipherText, char *secretKey, int numOf64)
{
    ...
    //预处理
    //numOf64 = strlen(cipherText) / 8 + (strlen(cipherText) / 8) ? 1 : 0;
    ...
    for (count = 0; count < 16; count++)
    {
        ...
        //*(deSonKey8_48 + 47 - count) = *(sonKey8_48+count);
    }
    ...
    {
        IniPermutate(cryptoText + count * 8);
        //CpChars(cryptoText, cipherText + count * 8,8);
...
//循环左移+置换选择2 16 times
        ...
        case 1:
        {
            ...
            *(tmp1Char_2) = (*(tmp7Char + 3) & binary_place[4]) >> 3;
            ...
            //tmp4Char_1 = (int *)tmp4Char_1;
            //*(tmp4Char_1) <<= 1;
            //*(tmp4Char_1) |= (int)*(tmp1Char_1);
            for (i = 0; i < 4; i++)
            {
                *(tmp4Char_1 + i) <<= 1;
            ...
            //tmp4Char_2 = (int *)tmp4Char_2;
            //*(tmp4Char_2) = (*(tmp4Char_2) << 1)&0x0fffffff;
            //*(tmp4Char_2) |= (int)*(tmp1Char_2);
            for (i = 0; i < 4; i++)
            {
                *(tmp4Char_2 + i) <<= 1;
        ...
        case 2:
        {
            ...
            *(tmp1Char_2) = (*(tmp7Char + 3) & 0x0c) >> 2;
            ...
            //tmp4Char_2 = (int *)tmp4Char_2;
            //*(tmp4Char_2) = (*(tmp4Char_2)<<2)&0x0fffffff;
            //*(tmp4Char_2) |= (int)*(tmp1Char_2);
            for (i = 0; i < 4; i++)
            {
                *(tmp4Char_2 + i) <<= 2;
        ...
        *(tmp7Char + 6) = *(tmp4Char_2 + 3);
        ...
        for (i = 0; i < 48; i++)
        {
            place = sub_key_permutation[i];
            row = place / 8;
            col = place % 8;
            *(tmp1Char_1) = *(tmp7Char + row);
            *(tmp1Char_1) &= binary_place[col - 1];
            *(sonKey8_48 + i / 8) |= ((col - 1 - i % 8) >= 0) ? (*tmp1Char_1 << (col - 1 - i % 8)) : (*tmp1Char_1 >> (i % 8 - col + 1));
        }
        */
...
void IniPermutate(unsigned char *plainText)
{
    ...
    unsigned char *res = (unsigned char *)malloc(8);
    for (i = 0; i < 8; i++)
    {
        *(res + i) = (char)0;
    }
    for (i = 0; i < 64; i++)
    {
        place = initial_message_permutation[i];
        row = place / 8;
        col = place % 8;
        *(tmp) = *(plainText + row);
        *(tmp) &= binary_place[col - 1];
        *(res + i / 8) |= ((col - 1 - i % 8) >= 0) ? (*tmp << (col - 1 - i % 8)) : (*tmp >> (i % 8 - col + 1));
    }
    Cp8chars(plainText, res);
    */
    ...
    /*
    void DepartLeftRight(unsigned char *cryptoText)
    { //将64bit信息左右替换
        /*
        cryptoText = (int *)cryptoText;
        int tmp;
        tmp = *(cryptoText);
        *(cryptoText) = *(cryptoText + 1);
        *(cryptoText + 1) = tmp;
        cryptoText = (unsigned char *)cryptoText;

        int i;
        unsigned char * tmp8Char = (unsigned char *)malloc(8);
        CpChars(tmp8Char,cryptoText,8);
        for(i=0;i<4;i++) {
            *(cryptoText+i) = *(tmp8Char+4+i);
            *(cryptoText+4+i) = *(tmp8Char+i);
        }
    }
    */
}
...
void DeIniPeimutate(unsigned char *cipherText)
{
    ...
    for (i = 0; i < 64; i++)
    {
        place = final_message_permutation[i];
        row = place / 8;
        col = place % 8;
        *(tmp) = *(cipherText + row);
        *(tmp) &= binary_place[col - 1];
        *(res + i) |= ((col - 1 - i % 8) >= 0) ? (*tmp << (col - 1 - i % 8)) : (*tmp >> (i % 8 - col + 1));
    }
    Cp8chars(cipherText, res);
    */
    unsigned char *tmpText = (unsigned char *)malloc(8);
    Permutate(cipherText, tmpText, final_message_permutation, 8);
    CpChars(cipherText, tmpText, 8);
    /*
    void Controller(unsigned char *cryptoText, unsigned char *sonKey8_48, int count)
    { //迭代控制器
        if (count != 0)
            DepartLeftRight(cryptoText);
        EnFFunc(cryptoText, sonKey8_48, count);
        count++;
        if (count < 16)
            Controller(cryptoText, sonKey8_48, count);
    }
    /*
    void Cp8chars(unsigned char *newChar, unsigned char *originChar)
    {
        int i;
        for (i = 0; i < 8; i++)
        {
